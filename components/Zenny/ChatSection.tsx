...
export const ChatSection: React.FC<ChatSectionProps> = ({
  ...
  useEffect(() => {
    if (messages.length > 0 || loading) {
      // Immediate scroll for new messages to reduce perceived delay
      scrollToBottom();

      // Also use requestAnimationFrame as backup for layout completion
  ...
  useEffect(() => {
    // Reduced timeout for faster scroll response
  ...
  const renderDeliveryStatus = (message: Message) => {
    ...
    return (
      <Ionicons name='checkmark' size={15} color='rgba(255,255,255,0.7)' />
    );
  };
      ...
      >
        ...
        {messages.map((message, idx) => {
          const dateLabel = message.timestamp
            ? getDateLabel(message.timestamp, message.isFromServer ?? true)
            : null;
          ...
          const isTTSAudioMessage =
            message.type === 'ai' && !!message.ttsAudioUrl;
                        ...
                        <View className='space-y-2'>
                          ...
                          {isTTSAudioMessage ? (
                            ...
                            <>
                              ...
                              {splitSentencesToLines(message.content || '').map(
                                (line, idx) => {
                                  ...
                                  if (
                                    ...
                                    return (
                                      <View
                                        key={idx}
                                        className='rounded-2xl w-fit text-md md:text-lg'
                                      >
