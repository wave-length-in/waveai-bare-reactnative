...
const ZennyMainDashboard: React.FC<ZennyMainDashboardProps> = ({
  ...
  const deliveryTimeoutsRef = useRef<
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      if (!hasFetchedHistoryRef.current) {
        ...
        console.log(
          '📜 Skipping chat history fetch - already fetched in this session'
        );
      ...
      try {
        ...
        if (userId) {
          trackMessageSent(
            userId,
            'audio',
            speechResult.DisplayText.length,
            characterId
          );
        }
  ...
  const handleSendMessage = useCallback(
    (content: string) => {
      ...
      // CRITICAL: Add message to UI immediately for instant feedback
      ...
      // Send to socket immediately (but after UI update)
      socket.emit('send_message', {
        userId,
        characterId,
        characterName,
      ...
      try {
        ...
        setMessages(prev => {
          ...
          console.log('🔊 Updated message with TTS audio:', {
            ...
            hasTtsAudio:
              !!updatedMessages[updatedMessages.length - 1]?.ttsAudioUrl,
            ttsProcessing:
              updatedMessages[updatedMessages.length - 1]?.ttsProcessing,
          });
